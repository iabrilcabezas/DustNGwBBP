...
base_folder=/global/cfs/cdirs/act/data/iabril/BBPower/230525_sample/sims/
...
for no_sim in {11265..19999}
do

echo ${no_sim}

python -m bbpower BBCompSep --cells_fiducial=${base_folder}/${rubbish} --cells_noise=${base_folder}/${rubbish} --cells_coadded=${base_folder}/${no_sim}/w/${name_run}_w_tot.fits --cells_coadded_cov=${base_folder}/${no_sim}/w/${name_run}_w_tot.fits --config=${config_file}_all.yml --output_dir=${base_folder}/${no_sim}/w/
python -m bbpower BBCompSep --cells_fiducial=${base_folder}/${rubbish} --cells_noise=${base_folder}/${rubbish} --cells_coadded=${base_folder}/${no_sim}/${w_type}/${name_run}_${w_type}_tot.fits --cells_coadded_cov=${base_folder}/${no_sim}/${w_type}/${name_run}_${w_type}_tot.fits --config=${config_file}_all.yml --output_dir=${base_folder}/${no_sim}/${w_type}/
