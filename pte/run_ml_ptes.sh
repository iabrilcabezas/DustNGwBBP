...
base_folder=/global/cfs/cdirs/act/data/iabril/BBPower/230503_pte/
...
for band in $bands
do
python -m bbpower BBCompSep --cells_fiducial=${base_folder}/${rubbish} --cells_noise=${base_folder}/${rubbish} --cells_coadded=${base_folder}/${no_sim}/${w_type}/${name_run}_${w_type}_tot.fits --config=${config_file}_${band}.yml --output_dir=${base_folder}/${no_sim}/${w_type}/${band}/
done
done